...
function inferStartId(latest){const mins=parseDrawMinutes(latest.time);if(mins==null)return latest.id;const sinceSix=mins>=360?mins-360:mins+1080;return latest.id-Math.floor(sinceSix/4)}
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    if(!control){const startId=inferStartId(latest);control=await createControl(cycleKey,startId)}
    const stored=await backfill(control,latest);
    const collectionEndId=Number(control.start_draw_id)+COLLECTION_DRAWS-1;
    const history=await db(`hotspot_draws?select=draw_id,draw_date,draw_time,numbers,bulls_eye&draw_id=gte.${control.start_draw_id}&draw_id=lte.${collectionEndId}&order=draw_id.asc&limit=${COLLECTION_DRAWS}`)||[];
    ...
    if(history.length<COLLECTION_DRAWS){return res.status(200).json({ok:true,mode:'preparing',collection:{have:history.length,need:COLLECTION_DRAWS,remaining:COLLECTION_DRAWS-history.length},latest:{id:latest.id,time:latest.time},stored,message:'Finishing the 12-hour backfill before selecting groups.',source:'California Lottery official'})}
    ...
    return res.status(200).json({ok:true,mode:'tracking',collection:{have:COLLECTION_DRAWS,need:COLLECTION_DRAWS,remaining:0},latest:{id:latest.id,time:latest.time},stored,activeGroups:groups.length,selected,processed:tracking.processed,details:tracking.details,source:'California Lottery official'});
