...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    const valid=rows.filter(r=>norm(r.numbers).length===20).slice(0,WINDOW).reverse();
    const latest=valid.at(-1);
    const coreRows=[];
    for(let i=0;i<valid.length;i++){
      const nums=norm(valid[i].numbers);
      if(CORE.every(n=>nums.includes(n))) coreRows.push({index:i,row:valid[i],nums});
    }
    const counts=new Map();
    for(const c of coreRows){
      const age=valid.length-c.index;
      const weight=1/age;
    ...
    return res.status(200).json({ok:true,window:WINDOW,latestDrawId:Number(latest?.draw_id||0),latestDate:latest?.draw_date||'',latestTime:latest?.draw_time||'',core:CORE,coreAppearances:coreRows.length,companions,ready,picks});
