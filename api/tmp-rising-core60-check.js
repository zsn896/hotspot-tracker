  ...
  try{
    ...
    for(let i=0;i<valid.length;i++){
      ...
      for(let a=0;a<18;a++) for(let b=a+1;b<19;b++) for(let c=b+1;c<20;c++){
        ...
        if(recent) s.rows.push({drawId:Number(valid[i].draw_id),time:valid[i].draw_time||'',nums});
      }
    }
    ...
    const focus=arr.find(s=>s.triple.join(',')==='15,30,74');
    if(focus && focus.rows.length){
      const counts=new Map();
      for(let i=0;i<focus.rows.length;i++){
        const r=focus.rows[i];
        const age=focus.rows.length-i;
        const w=1/age;
