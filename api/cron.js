...
const { runActiveDensitySpecial } = require('../lib/special-active');
const { runAdvanced } = require('../lib/advanced');
...
function normalizeFive(values) {
  return [...new Set((values || []).map(Number).filter(n => Number.isInteger(n) && n >= 1 && n <= 80))]
    .sort((a, b) => a - b);
}
...
async function buildGroupFiveWave(groupFive) {
  const group = normalizeFive(groupFive?.numbers);
  if (!groupFive?.active || group.length !== 5) return null;
  ...
  const control = controls.find(row =>
    /^AUTO_CONTROL_\d{4}-\d{2}-\d{2}$/.test(String(row?.name || ''))
  ) || null;

  if (!control?.start_draw_id) return analyzeGroupWave([], group);
  ...
  return analyzeGroupWave(draws, group);
}

function attachWaveToGroupFive(groupFive, wave) {
  if (!groupFive) return groupFive;
  ...
  return {
    ...
    waveStateAr: wave.stateAr || null,
    waveScore: Number.isFinite(Number(wave.score)) ? Number(wave.score) : null,
    waveAlert: Boolean(wave.alert),
    waveAlertLevel: wave.alertLevel || 'NONE',
    waveMessageAr: wave.messageAr || null
  };
}
...
async function precursorHistoryBackfill() {
  const edges = (
    await db('hotspot_draws?select=draw_id,draw_date&order=draw_id.asc&limit=1')
  ) || [];

  const oldest = edges[0] || null;
  if (!oldest?.draw_id) {
    return {
      ok: false,
      stored: 0,
  ...
  const oldestId = Number(oldest.draw_id);
  const startId = Math.max(1, oldestId - PRECURSOR_CRON_BACKFILL);
  const ids = Array.from({ length: oldestId - startId }, (_, index) => startId + index);
  ...
  return {
    ...
    reason: stored > 0 ? 'background-backfill-added' : 'background-backfill-no-results'
  };
}
...
async function precursorLiveDraws() {
  ...
  for (
    ...
    rows.push(...page);
    if (page.length < PRECURSOR_LIVE_PAGE_SIZE) break;
  }

  return rows
    .filter(draw =>
      Number.isFinite(Number(draw?.draw_id)) &&
      normalizeFive(draw?.numbers).length === 20
    )
    .slice(0, PRECURSOR_LIVE_MAX_DRAWS)
...
async function precursorLiveUpdate() {
  ...
  if (!groups.length) {
    return {
      ok: true,
      groups: [],
      reason: 'no-server-strong-manual-groups'
    };
  }

  const draws = await precursorLiveDraws();
  if (draws.length < 80) {
    return {
      ok: false,
  ...
  for (const group of groups) {
    const numbers = normalizeFive(group?.numbers);
    if (numbers.length < 3 || numbers.length > 5) continue;

    try {
      const analysis = analyzePrecursors(draws, numbers);
      results.push({
        groupId: group.id,
        name: group.name,
        numbers,
...
async function safeCloseDraw(id, batchMap) {
  const fromBatch = batchMap.get(Number(id));
  if (Number(fromBatch?.id) === Number(id)) return fromBatch;

  try {
    const direct = await getDraw(Number(id));
    if (Number(direct?.id) === Number(id)) return direct;
  } catch (_) {
...
async function getFinalTrackingDraw() {
  ...
  const latestMinutes = parseDrawMinutes(latest.time);
  if (latestMinutes != null && latestMinutes <= CLOSE_START_MINUTES) return latest;

  for (let offset = 1; offset <= 20; offset++) {
    try {
      const candidate = await getDraw(Number(latest.id) - offset);
      const minutes = parseDrawMinutes(candidate?.time);
      if (candidate?.id && minutes != null && minutes <= CLOSE_START_MINUTES) {
        return candidate;
      }
    } catch (_) {
...
async function getCloseGroups() {
  ...
  return rows.filter(g => {
    ...
    return (
      ...
      name.startsWith(STRONG_MANUAL_PREFIX)
    );
  });
}

async function finalizeOneCloseGroup(group, finalDraw) {
  const after = Number(
  ...
  const isManual =
    groupName.startsWith(MANUAL_PREFIX) ||
    groupName.startsWith(STRONG_MANUAL_PREFIX);
  ...
  for (const expectedId of ids) {
    const draw = await safeCloseDraw(expectedId, batchMap);

    if (!draw || Number(draw.id) !== Number(expectedId)) {
      stoppedAtMissing = Number(expectedId);
      break;
    }
    ...
    if (!isManual || Number(result?.count || 0) >= 3) {
...
async function finalizeTrackingClose() {
  const finalDraw = await getFinalTrackingDraw();
  ...
  if (!finalDraw?.id) {
    return {
      ...
      processed: 0,
      finalDrawId: null,
      error: 'Could not resolve the final official draw at or before 2:00 AM.'
    };
  }

  await storeCloseDraw(finalDraw);
  const groups = await getCloseGroups();
  const details = [];
  let processed = 0;

  for (const group of groups) {
    const result = await finalizeOneCloseGroup(group, finalDraw);
    details.push(result);
  ...
  return {
    ok: true,
    processed,
    finalDrawId: finalDraw.id,
    finalDrawTime: finalDraw.time,
    details
  };
}
...
module.exports = async (req, res) => {
  ...
  req.headers['x-worker-secret'] = process.env.WORKER_SECRET;
  const collector = createCollector();
  await worker(req, collector);

  const workerPayload = collector.payload || {
    ok: false,
    error: 'Worker returned no JSON payload'
  };
  ...
  if (collector.statusCode < 400 && workerPayload?.ok !== false) {
    try {
      precursorArchive = await precursorHistoryBackfill();
    } catch (e) {
      precursorArchive = {
        ok: false,
        stored: 0,
    ...
    try {
      precursorLive = await precursorLiveUpdate();
    } catch (e) {
      precursorLive = {
        ok: false,
        groups: [],
  ...
  if (
    ...
    workerPayload?.mode === 'idle'
  ) {
    try {
      closeFinalize = await finalizeTrackingClose();
    } catch (e) {
      closeFinalize = {
        ok: false,
        processed: 0,
  ...
  if (
    ...
    workerPayload?.ok !== false &&
    (
      workerPayload?.mode === 'collecting' ||
      workerPayload?.mode === 'preparing' ||
      workerPayload?.mode === 'tracking'
    )
  ) {
    try {
      groupFive = await runGroupFive();

      if (groupFive?.ok !== false && groupFive?.active) {
        try {
          const wave = await buildGroupFiveWave(groupFive);
          groupFive = attachWaveToGroupFive(groupFive, wave);
        } catch (waveError) {
          groupFive = {
            ...groupFive,
            wave: {
  ...
  if (
    ...
    try {
      specialActive = await runActiveDensitySpecial();
    } catch (e) {
      specialActive = {
        ok: false,
        replaced: false,
