const {db,statsForGroup,californiaNowParts,scheduleMode,parseDrawMinutes}=require('./lib');
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    let history=[];if(control){const rawHistory=await db(`hotspot_draws?select=draw_id,draw_date,draw_time,numbers,bulls_eye&draw_id=gte.${control.start_draw_id}&order=draw_id.asc&limit=220`)||[];history=rawHistory.filter(d=>inCollectionWindow(d,now))}
