...
function analyzeGroupWave(inputDraws, inputGroup) {
  ...
  const statisticalRecentEvidence =
    statisticalEvidenceRank(statistical20.evidence) >= 1;

  const statisticalPersistentEvidence =
    statisticalEvidenceRank(statistical40.evidence) >= 1;

  const strongStatisticalEvidence =
    statisticalRecentEvidence &&
    statisticalPersistentEvidence &&
    statistical20.liftVsRandom > 1 &&
    statistical40.liftVsRandom > 1;

  const strongWaveEvidence =
    w20.threePlus >= 4 &&
    score >= 68 &&
    strongCoreEvidence &&
    strongStatisticalEvidence &&
    (
      strongRotation ||
      w20.fourPlus >= 2
    );
  ...
  return {
    ...
    messageAr:
      state === 'STRONG'
        ? 'المجموعة داخل موجة قوية مع Core3 قائد واستمرارية فعلية ودليل إحصائي أعلى من العشوائية في آخر 20 و40 سحبة.'
        :
      state === 'REBOUND_WATCH'
        ? 'ظهرت عودة نشاط بعد هدوء، مع انتقال أو اتساع في قيادة ثلاثيات المجموعة.'
        :
      state === 'RISING'
        ? strongCoreEvidence && !strongStatisticalEvidence
