...
function buildHitTierForecast(draws, ranked, options, target, tierMetrics) {
  ...
  const validated4 = Boolean(
    fourEvidence &&
    Number(fourEvidence.validationSignals || 0) >= 4 &&
    Number(fourEvidence.validationHits || 0) >= 3 &&
    Number(fourEvidence.validationLift || 0) >= 1.35 &&
    Number(fourEvidence.discoveryHits || 0) >= 3
  );
  ...
  if (currentHits >= 3) {
    ...
    score = 12;
  } else if (validated4 && fourPlusScore >= 60) {
    status = 'STRONG';
    expectedTier = '4_PLUS';
    score = Math.max(fourPlusScore, 65);
  ...
  return {
    ...
    rule: 'STRONG requires a currently active precursor with validated 4+/5 behavior. MEDIUM reflects validated 3+ pressure. 5/5 can boost strength only when repeated discovery and validation evidence exists.',
...
function analyzePrecursors(inputDraws, targetNumbers, rawOptions = {}) {
  ...
  return {
    ok: true,
    model: 'HISTORICAL_PRECURSOR_VALIDATED_V3_HIT_TIERS',
    target,
    rule: 'A target HIT is any draw containing 3 or more target numbers. Candidate precursor triples are discovered only in the first 70% of history and validated on the later 30%. The same validated signals are then measured separately against 4+ and 5/5 outcomes. STRONG is reserved for validated 4+ pressure; MEDIUM represents validated 3+ pressure.',
