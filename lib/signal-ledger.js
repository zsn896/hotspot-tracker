...
async function ledgerReport(options = {}) {
  ...
  const recent = (await db('signal_episodes?select=id,target,status,start_draw_id,last_draw_id,window_end_draw_id,resolved,best_hit_count,created_at&order=id.desc&limit=50')) || [];
  ...
  return {
    ...
    recentEpisodes: recent.map(row => ({
      id: row.id, target: row.target, status: row.status,
      startDrawId: Number(row.start_draw_id), lastObservedDrawId: Number(row.last_draw_id),
      endDrawId: Number(row.window_end_draw_id), recordedAt: row.created_at || null,
      resolved: row.resolved === true,
      bestHitCount: row.resolved === true ? row.best_hit_count : null,
      matchesWindow: Number(row.window_end_draw_id) - Number(row.start_draw_id) === window,
      success: row.resolved === true && Number.isInteger(row.best_hit_count) && row.best_hit_count >= 0 && row.best_hit_count <= GROUP
        ? row.best_hit_count >= threshold : null
    })),
