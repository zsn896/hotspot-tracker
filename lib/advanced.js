...
async function initialBackfill(
  ...
  const draws = require('./draw-sequence').contiguousDraws(
    await db(`hotspot_draws?select=draw_id,numbers,bulls_eye&draw_id=gt.${start}&draw_id=lte.${end}&order=draw_id.asc`),
    start, end
  );
