'use strict';
const { cursorUpdatePath } = require('./tracking-cursor');

const { db, score } = require('../api/lib');
const { scoreCandidate: scoreProductionSignal } = require('./production-signal');
...
function deepAnalyze50(
  ...
  for (
    ...
    */
    ...
    const productionSignal = scoreProductionSignal(window, numbers);
    const adjustedScore = scoreValue + productionSignal.evidenceScore * 0.15;
  ...
  candidates.sort(
    (a, b) =>
      ...
      b.productionSignal.evidenceScore -
      a.productionSignal.evidenceScore
...
async function trackGroup(
  ...
  const pending = require('./draw-sequence').contiguousDraws(draws, after, target);
  ...
  await db(
    cursorUpdatePath(group, Number(lastSeen)),
    {
      method:
        'PATCH',
...
async function runGroupFive() {
  ...
  */
  for (
    ...
    */
    if (
      latestId < cutoff || !trackedNow.capReached
    ) {
      break;
    }
